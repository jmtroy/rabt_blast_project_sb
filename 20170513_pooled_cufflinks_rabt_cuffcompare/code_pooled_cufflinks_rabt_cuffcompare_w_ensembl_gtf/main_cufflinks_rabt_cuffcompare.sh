...
OUTPUT_DATA_FOLDER="$PROJECT_FOLDER"/output_code_pooled_cufflinks_rabt_cuffcompare_w_ensembl_gtf"_"$RUN_ID
...
MERGED_BAM=$OUTPUT_DATA_FOLDER/D30C_merged_accepted_hits.bam
...
CUFFLINKS_OUT="$OUTPUT_DATA_FOLDER"/D30C_cufflinks_pooled_rabt
...
cuffcompare -V -o "$CUFFCOMPARE_OUT" -C -s "$CHR_FA" -r "$gtf" -R "$GTF1" 2> "$OUTPUT_DATA_FOLDER"/D30c_contained.log.txt
...
MERGED_BAM=$OUTPUT_DATA_FOLDER/T30C_merged_accepted_hits.bam
...
CUFFLINKS_OUT="$OUTPUT_DATA_FOLDER"/T30C_cufflinks_pooled_rabt
cufflinks -p 12 -o $CUFFLINKS_OUT --GTF-guide $gtf $MERGED_BAM

# do cuffcompare (see notes below)
GTF1="$CUFFLINKS_OUT"/transcripts.gtf
CUFFCOMPARE_OUT="$OUTPUT_DATA_FOLDER"/T30C_cuffcompare_contained
cuffcompare -V -o "$CUFFCOMPARE_OUT" -C -s "$CHR_FA" -r "$gtf" -R "$GTF1" 2> "$OUTPUT_DATA_FOLDER"/T30c_contained.log.txt
